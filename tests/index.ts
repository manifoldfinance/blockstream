...
import {
  reconcileLogHistoryWithAddedBlock,
  reconcileLogHistoryWithRemovedBlock,
} from '../source/log-reconciler';
import {
  Block,
  Log,
  FilterOptions,
...
describe('reconcileBlockHistory', () => {
  ...
  const onBlockAdded = async (block: Block) => {
    await delay(0);
    newBlockAnnouncements.push(block);
  };
  const onBlockRemoved = async (block: Block) => {
    await delay(0);
  ...
  it('does not announce new block on repeat of current head', async () => {
    const oldHistory = Promise.resolve(
      ImmutableList<Block>([new MockBlock(0x7777)]),
    );
    const newBlock = new MockBlock(0x7777);
  ...
  it('announces a new head when nth block is added to history', async () => {
    const oldHistory = Promise.resolve(
      ImmutableList<Block>([new MockBlock(0x7777), new MockBlock(0x7778)]),
    );
  ...
  it('resets history if reconciliation not possible', async () => {
    ...
    expect(blockRemovalAnnouncments).to.deep.equal(
      (await oldHistory).reverse().toJS(),
    );
  });

  it('throws if block fetching of parent during backfill fails', async () => {
    const getBlockByHash = async (hash: string): Promise<Block | null> => {
      await delay(0);
      return null;
    };
    const oldHistory = Promise.resolve(
    ...
    const newHistoryPromise = reconcileBlockHistory(
      getBlockByHash,
      oldHistory,
      newBlock,
      onBlockAdded,
    ...
    await expect(newHistoryPromise).to.eventually.be.rejectedWith(
      Error,
...
describe('reconcileLogHistoryWithAddedBlock', async () => {
  ...
  const onLogsAdded = async (blockHash: string, logs: Array<Log>) => {
    await delay(0);
    logs.forEach((log) => newLogAnnouncements.push(log));
  };
  ...
  it('does not fetch logs if no filters are applied', async () => {
    let called = 0;
    const getLogs = async (filterOptions: FilterOptions): Promise<Log[]> => {
      ++called;
    ...
    const newLogHistory = await reconcileLogHistoryWithAddedBlock(
      getLogs,
      oldLogHistory,
      newBlock,
      onLogsAdded,
  ...
  it('adds block with logs', async () => {
    ...
    expect(newLogHistory.toJS()).to.deep.equal([
      new MockLog(
        '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7777',
      ),
    ]);
  ...
  it('fails if getLogs fails', async () => {
    ...
    await expect(newLogHistoryPromise).to.eventually.be.rejectedWith(
      Error,
      'apple',
    );
    expect(newLogAnnouncements).to.be.empty;
  ...
  it('fails if old block with logs is added before new block with logs is removed', async () => {
    ...
    const firstLogHistory = await reconcileLogHistoryWithAddedBlock(
      getLogs,
      oldLogHistory,
      firstBlock,
      onLogsAdded,
...
describe('reconcileLogHistoryWithRemovedBlock', async () => {
  ...
  const onLogsRemoved = async (blockHash: string, logs: Array<Log>) => {
    await delay(0);
    logs.forEach((log) => removedLogAnnouncements.push(log));
  };
  ...
  it('returns empty log history when starting with null log', async () => {
    ...
    const newLogHistory = await reconcileLogHistoryWithRemovedBlock(
      oldLogHistory,
      removedBlock,
      onLogsRemoved,
    );
  ...
  it('handles block removal with no associated logs', async () => {
    ...
    const oldLogHistory = Promise.resolve(
      ImmutableList<Log>([
        new MockLog(
          '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7776',
        ),
      ]),
    );
  ...
  it('removes logs at head for given block', async () => {
    ...
    const oldLogHistory = Promise.resolve(
      ImmutableList<Log>([
        new MockLog(
          '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7775',
        ),
        new MockLog(
          '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7776',
        ),
  ...
  it('throws if removed block is not at head', async () => {
    ...
    const newLogHistoryPromise = reconcileLogHistoryWithRemovedBlock(
      oldLogHistory,
      removedBlock,
      onLogsRemoved,
    );
...
describe('BlockAndLogStreamer', async () => {
  ...
  const onBlockAdded = (block: Block) =>
    announcements.push({ addition: true, item: block });
  const onBlockRemoved = (block: Block) =>
    announcements.push({ addition: false, item: block });
  const onLogsAdded = (blockHash: string, logs: Array<Log>) =>
    logs.forEach((log) => announcements.push({ addition: true, item: log }));
  const onLogsRemoved = (blockHash: string, logs: Array<Log>) =>
    logs.forEach((log) => announcements.push({ addition: false, item: log }));
  const onError = (error: any) =>
    announcements.push({ addition: true, item: error });

  const reinitialize = (
    getBlockByHash: (hash: string) => Promise<Block | null>,
    getLogs: (filterOptions: FilterOptions) => Promise<Log[]>,
  ) => {
    blockAndLogStreamer = new BlockAndLogStreamer(
      getBlockByHash,
      getLogs,
      onError,
      { blockRetention: 5 },
  ...
  it('announces removed blocks and logs', async () => {
    const logs = [
      ...
      new MockLog(
        '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7778',
        0,
        'AAAA',
      ),
      new MockLog(
  ...
  it('latest block is latest fully reconciled block', async () => {
    await blockAndLogStreamer.reconcileNewBlock(new MockBlock(0x7777));
    const promise = blockAndLogStreamer.reconcileNewBlock(
      new MockBlock(0x7779),
    );

    expect(blockAndLogStreamer.getLatestReconciledBlock()).to.deep.equal(
      new MockBlock(0x7777),
    );
    await promise;
    expect(blockAndLogStreamer.getLatestReconciledBlock()).to.deep.equal(
  ...
  it('adding multiple blocks in quick succession results in expected callbacks', async () => {
    const logs = [
      ...
      new MockLog(
        '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cAAAA7779',
        0,
        'AAAA',
      ),
      new MockLog(
  ...
  it('swallows errors from callbacks', async () => {
    ...
    blockAndLogStreamer.subscribeToOnBlockAdded((block) => {
      throw new Error('apple');
    });
    blockAndLogStreamer.subscribeToOnBlockRemoved((block) => {
      throw new Error('banana');
    });
    blockAndLogStreamer.subscribeToOnLogsAdded((logs) => {
      throw new Error('cherry');
    });
    blockAndLogStreamer.subscribeToOnLogsRemoved((logs) => {
      throw new Error('durian');
    });
  ...
  it('unsubscribes correctly', async () => {
    const addBlockToken = blockAndLogStreamer.subscribeToOnBlockAdded(
      (block) => expect(true).to.be.false,
    );
    blockAndLogStreamer.unsubscribeFromOnBlockAdded(addBlockToken);
    const removeBlockToken = blockAndLogStreamer.subscribeToOnBlockRemoved(
      (block) => expect(true).to.be.false,
    );
    blockAndLogStreamer.unsubscribeFromOnBlockRemoved(removeBlockToken);
    const addLogToken = blockAndLogStreamer.subscribeToOnLogsAdded(
      (block) => expect(true).to.be.false,
    );
    blockAndLogStreamer.unsubscribeFromOnLogsAdded(addLogToken);
    const removeLogToken = blockAndLogStreamer.subscribeToOnLogsRemoved(
      (block) => expect(true).to.be.false,
    );
    ...
    await blockAndLogStreamer.reconcileNewBlock(
      new MockBlock(0x7779, 'AAAA', 'BBBB'),
    );
  });
  ...
  it('throws if unsubscribing with invalid token', async () => {
    ...
    expect(() =>
      blockAndLogStreamer.unsubscribeFromOnBlockAdded(removeBlockToken),
    ).to.throw(Error);
    expect(() =>
      blockAndLogStreamer.unsubscribeFromOnBlockRemoved(addBlockToken),
    ).to.throw(Error);
    expect(() =>
      blockAndLogStreamer.unsubscribeFromOnLogsAdded(addBlockToken),
    ).to.throw(Error);
    expect(() =>
      blockAndLogStreamer.unsubscribeFromOnLogsRemoved(addBlockToken),
    ).to.throw(Error);
  });
  ...
  it("doesn't call getLogs if no filters are attached", async () => {
    ...
    const filterAToken = blockAndLogStreamer.addLogFilter({
      address: '0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef',
      topics: [],
    });
    const filterBToken = blockAndLogStreamer.addLogFilter({
      address: '0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef',
      topics: [
        '0xbadf00dbadf00dbadf00dbadf00dbadf00dbadf00dbadf00dbadf00dbaadf00d',
      ],
  ...
  it("does not announce or make changes to state if we can't fetch a parent block", async () => {
    ...
    const getBlockByHash = async (hash: string) =>
      hash ===
      '0xbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cbl0cBBBB7778'
        ? null
        : defaultGetBlockByHash(hash);
    reinitialize(getBlockByHash, getLogsFactory(1));

    await blockAndLogStreamer.reconcileNewBlock(new MockBlock(0x7777));
    await blockAndLogStreamer.reconcileNewBlock(new MockBlock(0x7778));
    await blockAndLogStreamer
      .reconcileNewBlock(new MockBlock(0x7779, 'BBBB', 'BBBB'))
      .catch(() => {});
  ...
  it('non-awaited reconciliation failure will result in failure of following reconciliation', async () => {
    ...
    blockAndLogStreamer
      ...
      .catch(() => {});
    await blockAndLogStreamer
      .reconcileNewBlock(new MockBlock(0x7779))
      .catch(() => {});
