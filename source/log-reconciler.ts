...
export const reconcileLogHistoryWithAddedBlock = async <
...
>(
  ...
  logHistory = await addNewLogsToHead(
    newBlock.hash,
    logHistory,
    logs,
    onLogsAdded,
...
const getFilteredLogs = async <TBlock extends Block, TLog extends Log>(
  getLogs: (filterOptions: FilterOptions) => Promise<Array<TLog>>,
  newBlock: TBlock,
...
const addNewLogsToHead = async <TLog extends Log>(
  blockHash: string,
  logHistory: LogHistory<TLog>,
  newLogs: Array<TLog>,
  onLogsAdded: (blockHash: string, logs: Array<TLog>) => Promise<void>,
): Promise<LogHistory<TLog>> => {
  const sortedLogs = newLogs.sort(
    (logA, logB) => parseHexInt(logA.logIndex) - parseHexInt(logB.logIndex),
  );
  ...
  for (const logToAdd of sortedLogs) {
    ...
    if (
      logHistory.some(
        (logInHistory) =>
          logInHistory!.blockHash === logToAdd.blockHash &&
...
const pruneOldLogs = async <TBlock extends Block, TLog extends Log>(
  logHistory: LogHistory<TLog>,
  newBlock: TBlock,
  ...
  return logHistory
    .skipUntil(
      (log) =>
        parseHexInt(newBlock.number) - parseHexInt(log!.blockNumber) <
        historyBlockLength,
    )
    .toList();
};

const ensureOrder = <TLog extends Log>(
  headLog: TLog | undefined,
  newLog: TLog,
) => {
  ...
  if (headBlockNumber > newLogBlockNumber)
    throw new Error(
      `received log for a block (${newLogBlockNumber}) older than current head log's block (${headBlockNumber})`,
    );
  ...
  if (headLogIndex >= newLogIndex)
    throw new Error(
      `received log with same block number (${newLogBlockNumber}) but index (${newLogIndex}) is the same or older than previous index (${headLogIndex})`,
    );
};

export const reconcileLogHistoryWithRemovedBlock = async <
  TBlock extends Block,
  TLog extends Log
>(
  logHistory: LogHistory<TLog> | Promise<LogHistory<TLog>>,
