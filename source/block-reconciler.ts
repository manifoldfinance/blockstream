...
const backfill = async <TBlock extends Block>(
  ...
  if (
    parseHexInt(parentBlock.number) + blockRetention <
    parseHexInt(blockHistory.last().number)
  )
    return await rollback(blockHistory, onBlockRemoved);
...
const isFirstBlock = <TBlock extends Block>(
  blockHistory: BlockHistory<TBlock>,
): boolean => {
...
const isOlderThanOldestBlock = <TBlock extends Block>(
  ...
  return (
    parseHexInt(blockHistory.first().number) > parseHexInt(newBlock.number)
  );
};

const isAlreadyInHistory = <TBlock extends Block>(
  blockHistory: BlockHistory<TBlock>,
  newBlock: TBlock,
): boolean => {
...
const isNewHeadBlock = <TBlock extends Block>(
  blockHistory: BlockHistory<TBlock>,
  newBlock: TBlock,
): boolean => {
...
const parentHashIsInHistory = <TBlock extends Block>(
  blockHistory: BlockHistory<TBlock>,
  newBlock: TBlock,
): boolean => {
  // `block!` is required until the next version of `immutable` is published to NPM (current version 3.8.1) which improves the type definitions
