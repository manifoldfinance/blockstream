...
export class BlockAndLogStreamer<TBlock extends Block, TLog extends Log> {
  ...
  private blockHistory: Promise<BlockHistory<TBlock>> = Promise.resolve(
    this.lastKnownGoodBlockHistory,
  );
  private lastKnownGoodLogHistory: LogHistory<TLog> = ImmutableList<TLog>();
  private logHistory: Promise<LogHistory<TLog>> = Promise.resolve(
    this.lastKnownGoodLogHistory,
  );
  ...
  private readonly onBlockAddedSubscribers: {
    [propName: string]: (block: TBlock) => void;
  } = {};
  private readonly onBlockRemovedSubscribers: {
    [propName: string]: (block: TBlock) => void;
  } = {};
  private readonly onLogsAddedSubscribers: {
    [propName: string]: (blockHash: string, logs: Array<TLog>) => void;
  } = {};
  ...
  private readonly onBlockAdded = async (block: TBlock): Promise<void> => {
    ...
    const logFilters = Object.keys(this.logFilters).map(
      (key) => this.logFilters[key],
    );
  ...
  private readonly onBlockRemoved = async (block: TBlock): Promise<void> => {
    this.logHistory = reconcileLogHistoryWithRemovedBlock(
      this.logHistory,
      block,
      this.onLogsRemoved,
    );
  ...
  private readonly onLogsAdded = async (
    blockHash: string,
    logs: Array<TLog>,
  ): Promise<void> => {
    Object.keys(this.onLogsAddedSubscribers)
      ...
      .map((callback) => logAndSwallowWrapper(callback, this.onError))
      .forEach((callback) =>
        this.pendingCallbacks.push(() => callback(blockHash, logs)),
      );
  };

  private readonly onLogsRemoved = async (
    blockHash: string,
    logs: Array<TLog>,
  ): Promise<void> => {
  ...
  public readonly getLatestReconciledBlock = (): TBlock | null => {
    return this.lastKnownGoodBlockHistory.isEmpty()
      ? null
      : this.lastKnownGoodBlockHistory.last();
  };
  ...
  public readonly subscribeToOnBlockAdded = (
    onBlockAdded: (block: TBlock) => void,
  ): string => {
  ...
  public readonly unsubscribeFromOnBlockAdded = (token: string) => {
    if (!token.startsWith('on block added token '))
      throw new Error(
        `Expected a block added subscription token.  Actual: ${token}`,
      );
    delete this.onBlockAddedSubscribers[token];
  };

  public readonly subscribeToOnBlockRemoved = (
  ...
  public readonly subscribeToOnLogsAdded = (
    onLogsAdded: (blockHash: string, logs: Array<TLog>) => void,
  ): string => {
  ...
  public readonly unsubscribeFromOnLogsAdded = (token: string) => {
    if (!token.startsWith('on log added token '))
      throw new Error(
        `Expected a log added subscription token.  Actual: ${token}`,
      );
    delete this.onLogsAddedSubscribers[token];
  };

  public readonly subscribeToOnLogsRemoved = (
    onLogsRemoved: (blockHash: string, logs: Array<TLog>) => void,
  ): string => {
...
function logAndSwallowWrapper<T, U>(
